...
let setThemeSetting = (themeSetting, withTransition = false) => {
  ...
  if (checkbox) {
    checkbox.checked = determineComputedTheme() === "dark";
  }
...
let initTheme = () => {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const checkbox = document.getElementById("theme-toggle-checkbox");
    if (checkbox) {
      // Set checkbox state (CSS is driven by html[data-theme-setting], not checkbox)
      checkbox.checked = determineComputedTheme() === "dark";

      // Listen for checkbox toggle — enable animation only on user click
      checkbox.addEventListener("change", function () {
        const wrapper = checkbox.closest(".theme-toggle-wrapper");
        if (wrapper) wrapper.classList.add("theme-toggle-animate");
        setThemeSetting(this.checked ? "dark" : "light", true);
        // Remove animation class after transition ends
        setTimeout(() => {
          if (wrapper) wrapper.classList.remove("theme-toggle-animate");
        }, 350);
      });
    }
  });

  // Re-sync checkbox when theme changes (e.g., system preference)
